...
[PCI Express 6.0 规范](https://pcisig.com/pci-express-6.0-specification)

近二十年来，PCI Express ® (PCIe ® ) 规范一直是事实上的互连选择。 PCIe 6.0 规范将 PCIe 5.0 规范 (32 GT/s) 的带宽和能效提高了一倍，同时继续满足行业对高速、低延迟互连的需求。 PCIe 6.0 技术是一种经济高效且可扩展的互连解决方案，适用于数据中心、人工智能/机器学习、HPC、汽车、物联网和军事/航空航天等数据密集型市场。

PCIe 6.0 规格特性

- 64 GT/s 原始数据速率，通过 x16 配置高达 256 GB/s
- 具有 4 级 (PAM4) 信号的脉冲幅度调制，利用业界现有的 PAM4
- 轻量级前向纠错 (FEC) 和循环冗余校验 (CRC) 缓解了与 PAM4 信令相关的误码率增加
- 基于 Flit（流量控制单元）的编码支持 PAM4 调制，并与 FEC 和 CRC 结合使用，实现双倍带宽增益
- 更新了 Flit 模式中使用的数据包布局，以提供附加功能并简化处理
- 保持与所有前几代 PCIe 技术的向后兼容性

## lspci

- `-v, --verbose:` 选项开启后，将提供更为详尽的信息，其中涵盖了每个设备的内存地址区间以及相应的IRQ信息。
...
1. 内存空间BAR（Memory Space BAR），映射设备的内存空间，使主机能够访问设备的寄存器和内存缓冲区。
   - **32位内存空间BAR**：
...
2. I/O空间BAR（I/O Space BAR），映射设备的I/O端口，使主机能够访问设备的I/O寄存器。
